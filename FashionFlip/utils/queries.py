...
#         meta_batch = [" ".join(x) for x in meta_batch.loc[:, ~meta_batch.columns.isin(
#             ['id', 'year'])].values.tolist()]
#         # extract image batch
...
def make_query(query_sparse, query_dense, model=load_model(), bm25=create_sparse_embeddings(), index=index, images=create_images_and_metadata()[0], alpha=0.05):
...
def make_query_from_images(img, query_sparse=None):
    if query_sparse == None:
        ...
        output = make_query(query_sparse=query_sparse,
                            query_dense=img, alpha=0.5)
    return output
